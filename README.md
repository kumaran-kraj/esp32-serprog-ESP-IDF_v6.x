# esp32-serprog for ESP-IDF v6.x (v6.0.1 tested)
...
![esp32 c3 super mini to 25 series spi nor flash wiring diagram](.github/esp32c3supermini_serprog.jpg)  
...
* Should be compatible with all ESP32 family MCUs *(verified on ESP32-C3-SuperMini)*
...
if you are using vscode , install the [esp-idf-extension](https://marketplace.visualstudio.com/items?itemName=espressif.esp-idf-extension)
...
but then I would suggest UART to make life easier for flashing code to the ESP32
...
but it is supported only by a subset of USB Serial/JTAG-capable chips (e.g. ESP32-S3 but not ESP32-C3/C6, I tried and yep, can verify)
...
see flashrom for windows [flashrom win git](https://github.com/therealdreg/flashrom_build_windows_x64)


identify a programmer&chip (no action)

```cmd
flashrom -p serprog:dev=com1:921600,spispeed=8M
```

read to file

```cmd
flashrom -p serprog:dev=com1:921600,spispeed=8M -c <chip> -r flash.bin
```

erase chip

```cmd
flashrom -p serprog:dev=com1:921600,spispeed=8M -c <chip> --erase
```

write from file

```cmd
flashrom -p serprog:dev=com1:921600,spispeed=8M -c <chip> -w flash.bin
```


## License

since I have forked code from [thisiseth/esp32-serprog](https://github.com/thisiseth/esp32-serprog)
and he has borrowed snippets from the [pico-serprog](https://github.com/stacksmashing/pico-serprog) by [stacksmashing](https://github.com/stacksmashing), 

"the least common denominator is GPL 3.0" - thisiseth
